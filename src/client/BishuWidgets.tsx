...
export function BishuWidgets({ config }: { config: SiteConfig | null }) {
  const { customerId, chatbotEnabled, ga4MeasurementId, consoleUrl } =
    config ?? ({} as Partial<SiteConfig>);
  ...
  // ── 챗봇 ──
  useEffect(() => {
    if (!chatbotEnabled || !customerId || !consoleUrl) return;
