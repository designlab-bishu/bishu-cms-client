...
function matchesAccept(
  ...
  return accept.some((entry) => {
    ...
    if (value.startsWith(".")) return ext === value;
    if (value.endsWith("/*")) return contentType.startsWith(value.slice(0, -1));
 ...
 */
async function directUploadFormFiles(
  formData: FormData
  ...
  try {
    ...
    const field = (form.fields ?? []).find(
      (f: { id: string; type: string }) => f.id === fieldId
    );
    if (!field || field.type !== "file") {
      return { success: false, error: "잘못된 첨부 필드입니다." };
    }

    const maxFiles: number = field.file?.maxFiles ?? 1;
    const maxSizeMB: number = field.file?.maxSizeMB ?? 10;
    const accept: string[] | undefined = field.file?.accept;
...
export async function uploadFormFiles(
  ...
  if (!isApiMode()) return directUploadFormFiles(formData);

  const formSlug = (formData.get("formId") as string) || "";
  const rawFiles = formData.getAll("files") as File[];
