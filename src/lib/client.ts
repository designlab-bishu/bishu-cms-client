...
/**
 * Next.js 가 확장한 fetch 옵션. 패키지가 next 를 의존성으로 갖지 않으므로
 * 타입만 여기서 선언한다. Next 밖에서 실행되면 이 필드는 무시된다.
 */
...
function requireEnv(name: string): string {
  ...
  if (!value) {
    throw new CmsConfigError(
      `[cms-client] ${name} 환경변수가 설정되지 않았습니다.`
    );
  }
  return value;
}
 ...
 */
export async function fetchFromConsole<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `[cms-client] 콘솔 응답 오류 ${res.status} (${params.resource}): ${body.slice(0, 200)}`
      );
      return null;
    }
 ...
 */
export async function postToConsole<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `[cms-client] 쓰기 실패 ${res.status} (${body.action}): ${text.slice(0, 200)}`
      );
